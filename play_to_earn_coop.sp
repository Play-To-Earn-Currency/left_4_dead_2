...
public void OnPluginStart()
{
    ...
    if (GetCommandLine(commandLine, sizeof(commandLine)))
    {
        ...
        if (StrContains(commandLine, "-pteCoop 1") == -1)
        {
            PrintToServer("[PTE Coop] Will not be initialized, 'pteCoop' is not '1'");
            return;
        }
        ...
        // Get config path
        {
            ...
            if (start != -1)
            {
                ...
                PrintToServer("[PTE Coop] Config path: %s", configPath);
            }
            else
            {
                configPath = "addons/sourcemod/configs/play_to_earn_versus.cfg";
                PrintToServer("[PTE Coop] No -configPath provided using default %s", configPath);
            }
        }
    }
    ...
    // Round ended
    HookEventEx("map_transition", RoundEnd, EventHookMode_Post);
...
public void RoundEnd(Event event, const char[] name, bool dontBroadcast)
{
    ...
    for (int i = 0; i < MAXPLAYERS; i++)
    {
        ...
        IncrementWallet(client, pteEarnOnRoundWin, pteShowOnRoundWin, " PTE , by surviving");
    }
}
